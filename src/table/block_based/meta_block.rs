...
pub async fn read_properties(
    ...
    let mut x = 0;
    if let Some(v) = tmp_properties.get(PROPERTIES_DATA_SIZE) {
        if let Some(v) = get_var_uint64(v, &mut x) {
            properties.data_size = v;
        }
    };
    if let Some(v) = tmp_properties.get(PROPERTIES_INDEX_SIZE) {
        if let Some(v) = get_var_uint64(v, &mut x) {
            properties.index_size = v;
        }
    };
    if let Some(v) = tmp_properties.get(PROPERTIES_NUM_ENTRIES) {
        if let Some(v) = get_var_uint64(v, &mut x) {
            properties.num_entries = v;
        }
    };
    if let Some(v) = tmp_properties.get(PROPERTIES_DELETED_KEYS) {
        if let Some(v) = get_var_uint64(v, &mut x) {
